...
import { HexElement, ProcessedIncantation, ProcessedIngredient } from './types';
...
export const SPELL_TYPES: Record<
  string,
  { power: number; duration: number; complexity: number }
> = {
  spell: {
    power: 2,
    duration: 1,
 ...
 */
export function calculateElementalRelationship(
  element1Id: HexElement,
  element2Id: HexElement
): number {
...
export function processIngredient(ingredient: Ingredient): ProcessedIngredient {
  ...
  const elementValue =
    elementId && ELEMENTS[elementId.toUpperCase()]
      ? ELEMENTS[elementId.toUpperCase()].baseValue
      : 3;
 ...
 */
export function processIncantation(
  incantation: Incantation
): ProcessedIncantation {
  // Get the element if it exists, otherwise leave as undefined
  const elementId = incantation.affinity
    ? (incantation.affinity as HexElement)
    : undefined;
