import { Ingredient, Incantation, ElementInteraction, SpellVisualizationData } from '@astra-arcana/spellcasting-types';
import { HexElement } from './types';
import { processIngredient, processIncantation, calculateElementalRelationship, ELEMENTS } from './core';
...
export function generateVisualizationData(
  ...
  const elementIds = Object.keys(elementalBalance).filter(id => id !== 'neutral') as HexElement[];
  
  for (let i = 0; i < elementIds.length; i++) {
    for (let j = i + 1; j < elementIds.length; j++) {
      // Only consider elements that are actually present in the spell
      if (elementalBalance[elementIds[i]] > 0 && elementalBalance[elementIds[j]] > 0) {
        const element1 = elementIds[i];
        const element2 = elementIds[j];
        const relationshipValue = calculateElementalRelationship(element1, element2);
        
        // Weighted by the amount of each element present
        const interactionStrength = (elementalBalance[element1] + elementalBalance[element2]) / 2;
        ...
        } else if (ELEMENTS[element1.toUpperCase()]?.neighbors.includes(element2)) {
